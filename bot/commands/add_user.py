...
async def add_user_name(message:types.Message, state: FSMContext)->None:
    ...
    builder.adjust(1)
    await message.answer(text='Выберите роль пользователя:', reply_markup=builder.as_markup())
...
async def add_user_role(call:types.CallbackQuery, state: FSMContext)->None:
    ...
    builder.adjust(1)
    await call.message.answer('Создать пользователя с параметрами:', reply_markup=builder.as_markup())
...
async def finish(call:types.CallbackQuery,session_maker:sessionmaker, state:FSMContext)->None:
    ...
    if call.data.split(':')[1] == 'False':
        try:
            ...
            await call.message.answer(text='Ошибка, попробуйте еще раз')
            await state.clear()
    elif call.data.split(':')[1] == 'user_name':
        await call.message.answer(text='Выберите роль пользователя:')
        ...
        builder.button(text='Cooperator', callback_data='role:copywriter')
        builder.adjust(1)
        await call.message.answer(text='Выберите роль пользователя:', reply_markup=builder.as_markup())
        await state.set_state(AddUserState.role)
...
async def update_user_name(message:types.Message, state: FSMContext)->None:
    try:
        ...
        builder.button(text='Save', callback_data=f"update:False")
        builder.adjust(1)
        await message.answer('Создать пользователя с параметрами:', reply_markup=builder.as_markup())
        await state.set_state(AddUserState.finish)
    except:
