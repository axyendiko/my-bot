...
async def start(message:types.Message, session_maker:sessionmaker)->None:
    ...
    if user and user.role == 'administrator':
        ...
        builder.button(text='Add user', callback_data='function:addUser')
        builder.adjust(1)
        ...
        await message.answer(text=f'HI {user.user_name},select option \ncommands: \n /active - list of active users \n /inactive - list of inactive users', reply_markup=builder.as_markup())
    else:
